...
<xsl:stylesheet
  ...
  <xsl:template name="links_commands">
    ...
    <form id="form_search" name="form_search" action="{$cgi_base}/search/">
      <input tabindex="100" name="q" id="q" value="{@sg:searchTerms}" autofocus="autofocus" type="text" placeholder="🔍 Suche Wort oder #Tag..." class="awesomplete" data-multiple="true"/>
    </form>

    <form id="form_post" name="form_post" class="hidden-logged-out" action="{$cgi_base}">
      <input tabindex="300" name="post" type="text" placeholder="Was gibt's #Neues? (Notiz oder URL)"/>
    </form>
  </xsl:template>
